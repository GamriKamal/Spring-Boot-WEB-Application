...
//        model.addAttribute("movieList", discoverLists.discoverMovieList());
    ...
    public String addValues(@RequestParam("username") String userName, @RequestParam("email") String email,
                            @RequestParam("phone") String phoneNumber, @RequestParam("password") String password, @RequestParam("confirm-password") String confirmPassword) throws NoSuchAlgorithmException {
        Long uniqueID = java.util.UUID.randomUUID().getMostSignificantBits() & Long.MAX_VALUE;
